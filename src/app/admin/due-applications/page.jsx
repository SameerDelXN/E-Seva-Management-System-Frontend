...
export default function DueApplications() {
  ...
  const [sortBy, setSortBy] = useState('daysOverdue'); // Default sort by days overdue
  const [sortDirection, setSortDirection] = useState('desc'); // Default sort direction
  const [activeFilter, setActiveFilter] = useState('all'); // Track active filter: 'all', 'critical', 'moderate', 'low'
  ...
  const fetchDueApplications = async () => {
    try {
      ...
      applyFilter(activeFilter, sortedApps);
  ...
  const applyFilter = (filter, apps = applications) => {
    ...
    switch (filter) {
      case 'critical':
        filtered = apps.filter(app => app.daysOverdue > 7);
        break;
      case 'moderate':
        filtered = apps.filter(app => app.daysOverdue >= 3 && app.daysOverdue <= 7);
        break;
      case 'low':
        filtered = apps.filter(app => app.daysOverdue < 3);
        break;
      case 'all':
      default:
        filtered = [...apps];
  ...
  // Handle clicking on a stat card
  ...
  const handleSort = (field) => {
    ...
    const sorted = sortApplications(applications, field, newDirection);
    setApplications(sorted);
    
    // Apply filter to sorted applications
  ...
  const deleteApplication = async (id) => {
    ...
    try {
      ...
      const updatedFilteredApplications = filteredApplications.filter(app => app._id !== id);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard 
            ...
            value={stats.total} 
            icon={<FiClock className="h-6 w-6 text-blue-500" />}
            color={activeFilter === 'all' ? "bg-blue-200 border-2 border-blue-400" : "bg-blue-100"}
          ...
          <StatCard 
            ...
            value={stats.critical} 
            icon={<FiAlertTriangle className="h-6 w-6 text-red-500" />}
            color={activeFilter === 'critical' ? "bg-red-200 border-2 border-red-400" : "bg-red-100"}
            subtitle=">7 days overdue"
          ...
          <StatCard 
            ...
            value={stats.moderate} 
            icon={<FiAlertTriangle className="h-6 w-6 text-orange-500" />}
            color={activeFilter === 'moderate' ? "bg-orange-200 border-2 border-orange-400" : "bg-orange-100"}
            subtitle="3-7 days overdue"
            onClick={() => handleStatCardClick('moderate')}
            isActive={activeFilter === 'moderate'}
          />
          <StatCard 
            title="Low Priority" 
            value={stats.recent} 
            icon={<FiAlertTriangle className="h-6 w-6 text-yellow-500" />}
            color={activeFilter === 'low' ? "bg-yellow-200 border-2 border-yellow-400" : "bg-yellow-100"}
            subtitle="<3 days overdue"
        ...
        <div className="mt-8">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-gray-900">
              ...
              {activeFilter === 'critical' && 'Critical Priority Applications'}
              {activeFilter === 'moderate' && 'Moderate Priority Applications'}
              {activeFilter === 'low' && 'Low Priority Applications'}
            </h2>
            <div className="flex items-center space-x-4">
              <span className="text-gray-600">Total: {filteredApplications.length}</span>
              <button 
                onClick={fetchDueApplications}
                className="flex items-center bg-indigo-100 hover:bg-indigo-200 text-indigo-800 px-3 py-1 rounded"
              >
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredApplications.length === 0 ? (
                      <tr>
                        <td colSpan="10" className="px-6 py-4 text-center text-gray-500">
                          No {activeFilter !== 'all' ? activeFilter + ' priority' : 'overdue'} applications found
                        </td>
                      </tr>
                    ) : (
...
// StatCard Component
function StatCard({ title, value, icon, color, subtitle, onClick, isActive }) {
  return (
    <div 
      className={`${color} rounded-lg shadow-sm overflow-hidden cursor-pointer transition-all hover:shadow-md`}
      onClick={onClick}
    >
      <div className="p-5">
        <div className="flex items-center">
